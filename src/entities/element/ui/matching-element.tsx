...
const MatchingElement: React.FC<MatchingElementProps> = ({
  ...
  const [lineCoords, setLineCoords] = useState<{
    [key: string]: { start: Point; end: Point }
  }>({})
  ...
  const [prevCorrectPairs, setPrevCorrectPairs] = useState(
    element.data.correctPairs
  )

  if (
    element.id !== prevElementId ||
    isInteractive !== prevInteractive ||
    JSON.stringify(element.data.correctPairs) !==
      JSON.stringify(prevCorrectPairs)
  ) {
  ...
  const colors = useMemo(
    () => ["#6366f1", "#ec4899", "#22c55e", "#eab308", "#a855f7", "#f97316"],
    []
  )
  ...
  const handleItemClick = (id: string, side: "left" | "right") => {
    ...
    if (side === "left") {
      ...
      if (nextLeft && rightSelected) {
        setMatches((prev) => {
          const filtered = prev.filter(
            ([l, r]) => l !== nextLeft && r !== rightSelected
          )
      ...
      if (leftSelected && nextRight) {
        setMatches((prev) => {
          const filtered = prev.filter(
            ([l, r]) => l !== leftSelected && r !== nextRight
          )
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isSelected
                  ? "scale-[1.02] border-indigo-500 bg-indigo-500 text-white shadow-lg"
                  : matchedColor
            ...
            >
              ...
              <div
                className={cn(
                  "absolute top-1/2 -right-2 h-2.5 w-2.5 -translate-y-1/2 rounded-full border-2 border-white shadow-sm transition-colors duration-300",
                  isSelected
                    ? "bg-indigo-500"
                    : matchedColor
                      ? "bg-white"
          ...
          return (
            <div
              ...
              className={cn(
                "group relative cursor-pointer rounded-xl border p-4 text-center text-sm font-medium transition-all duration-300",
                isSelected
                  ? "scale-[1.02] border-pink-500 bg-pink-500 text-white shadow-lg"
                  : matchedColor
                    ? "border-transparent text-white shadow-md"
                    : "border-slate-200 bg-slate-50 text-slate-600 hover:border-slate-300 hover:bg-slate-100"
              )}
            ...
            >
              ...
              <div
                className={cn(
                  "absolute top-1/2 -left-2 h-2.5 w-2.5 -translate-y-1/2 rounded-full border-2 border-white shadow-sm transition-colors duration-300",
                  isSelected
                    ? "bg-pink-500"
                    : matchedColor
                      ? "bg-white"
